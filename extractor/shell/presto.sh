#!/bin/bash
STORAGE_BUCKET=S3:bi-dafiti-group
SCHEMA_NAME="RAW_SAP_EWP_DAFITI_BR"
TABLE_NAME="SCWM_ORDIM_C"
GLOVE_HOME=/home/etl/glove
GLOVE_TEMP=/home/etl/tmp_files
GLOVE_METADATA=/home/etl/metadata
GLOVE_NAMED_QUERIES=/home/etl/named_query
GLOVE_STORARE_BUCKET_STAGING=bi-dafiti-group-glove
GLOVE_STORARE_BUCKET_DISASTER_RECOVERY=bi-dafiti-group-glove

STORAGE_BUCKET_PREFIX=$(echo ${STORAGE_BUCKET}|cut -d ':' -f 1)

# Select Storage Bucket Type
storage_bucket_type(){
   if [[ "$1" == *"S3:"* ]] || [[ "${STORAGE_BUCKET_PREFIX}" == "S3" ]]
    then 
     echo "STORAGE BUCKET SET TO S3"
     STORAGE_BUCKET_PREFIX="S3://"
     STORAGE_BUCKET=$(echo ${STORAGE_BUCKET}|cut -d ':' -f 2)
   elif [[ "$1" == *"AZS:"* ]]
    then  
      echo "STORAGE BUCKET SET TO AZURE BLOB STORAGE"
      STORAGE_BUCKET_PREFIX="/"
      STORAGE_BUCKET=$(echo ${STORAGE_BUCKET}|cut -d ':' -f 2)
   elif [[ "$1" == *"GDS:"* ]]
    then 
      echo "STORAGE BUCKET SET TO GOOGLE STORAGE"
      STORAGE_BUCKET_PREFIX="/"
      STORAGE_BUCKET=$(echo ${STORAGE_BUCKET}|cut -d ':' -f 2)
   elif [[ "$1" == *"AZS:"* ]]
    then 
      echo "STORAGE BUCKET SET TO HUAWEI STORAGE"
      STORAGE_BUCKET_PREFIX="/"
      STORAGE_BUCKET=$(echo ${STORAGE_BUCKET}|cut -d ':' -f 2)
   elif [[ "$1" == *"FILE:"* ]]
    then 
      ...
      STORAGE_BUCKET_PREFIX="/"
      STORAGE_BUCKET=$(echo ${STORAGE_BUCKET}|cut -d ':' -f 2)
    elif [[ -z ${STORAGE_BUCKET_PREFIX} ]] 
     then
      echo "STORAGE BUCKET SET TO DEFAULT STORAGE (S3)"
      STORAGE_BUCKET_PREFIX="S3://"
      STORAGE_BUCKET=$(echo ${STORAGE_BUCKET}|cut -d ':' -f 2)
    else 
      echo "STORAGE BUCKET SET TO DEFAULT STORAGE (S3)"
...
#Set Storage Bucket Type
storage_bucket_type "${STORAGE_BUCKET}"

# Caminho do bucket.
STORAGE_QUEUE_PATH="${STORAGE_BUCKET_PREFIX}${STORAGE_BUCKET}/${SCHEMA_HASH}_${SCHEMA}/${ENTITY_HASH}_${TABLE}/rawfile/queue/"
STORAGE_DISASTER_RECOVERY_QUEUE_PATH="${STORAGE_BUCKET_PREFIX}${GLOVE_STORARE_BUCKET_DISASTER_RECOVERY}/disaster_recovery/${SCHEMA_HASH}_${SCHEMA}/${ENTITY_HASH}_${TABLE}/rawfile/queue/"
STORAGE_STAGING_QUEUE_PATH="${STORAGE_BUCKET_PREFIX}${GLOVE_STORARE_BUCKET_STAGING}/staging"
...
backup()
{
    ...
    STORAGE_BACKUP_METADATA_PATH="${STORAGE_BUCKET_PREFIX}${STORAGE_BUCKET_BACKUP}/backup/${SCHEMA}/${TABLE}/metadata/year=${YEAR}/month=${MONTH}/day=${DAY}/hour=${HOUR}/minute=${MINUTE}/"
    STORAGE_BACKUP_QUEUE_PATH="${STORAGE_BUCKET_PREFIX}${STORAGE_BUCKET_BACKUP}/backup/${SCHEMA}/${TABLE}/rawfile/year=${YEAR}/month=${MONTH}/day=${DAY}/hour=${HOUR}/minute=${MINUTE}/"
