		...
		if [ ${IS_EXPORT} = 1 ]; then
			...
			if [ "${#EXPORT_BUCKET}" -gt "0" ]; then
				...
				if [ "${#EXPORT_SHEETS_RECIPIENTS}" -gt "0" ] && [ "${#EXPORT_SHEETS_SUBJECT}" -gt "0" ]; then
					...
					echo "Your file is available for download for 7 days: ${LINK_PRESIGN}. Generated by Glove, a modular data integration platform." | mutt -s "${EXPORT_SHEETS_SUBJECT}" -b ${EXPORT_SHEETS_RECIPIENTS}
				fi				
			else
