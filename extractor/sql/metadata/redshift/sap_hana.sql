    ...
    FROM (
        ...
        SELECT DISTINCT
			...
			CASE DATA_TYPE_NAME
				...
				WHEN 'VARCHAR'		THEN 'varchar'||'(' || TO_BIGINT(CASE WHEN COALESCE("LENGTH", 0) = 0 THEN 255 ELSE "LENGTH" END) ||')'
				WHEN 'VARBINARY'	THEN 'varchar'||'(' || TO_BIGINT(CASE WHEN COALESCE("LENGTH", 0) = 0 THEN 255 ELSE "LENGTH" END) ||')'
				WHEN 'NVARCHAR'		THEN 'varchar'||'(' || TO_BIGINT(CASE WHEN COALESCE("LENGTH", 0) = 0 THEN 255 ELSE "LENGTH" END) ||')'
				WHEN 'CHAR'			THEN 'varchar'||'(' || TO_BIGINT(CASE WHEN COALESCE("LENGTH", 0) = 0 THEN 255 ELSE "LENGTH" END) ||')'
